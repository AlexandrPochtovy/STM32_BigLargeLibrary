...
void I2C_Start_DMA(I2C_DMA_Conn_t *_i2c) {
	_i2c->status = PORT_BUSY;
	LL_I2C_DisableDMAReq_TX(_i2c->i2c);	//disable DMA req while setting
	LL_I2C_DisableIT_BUF(_i2c->i2c);		//disable interrupt for DMA work
	if (_i2c->len > 1) {
		LL_I2C_AcknowledgeNextData(_i2c->i2c, LL_I2C_ACK); 	// Ack enable if more one bytes read
	}	else {
		LL_I2C_AcknowledgeNextData(_i2c->i2c, LL_I2C_NACK);//Ack disable if only one byte read
	}
	LL_DMA_DisableChannel((DMA_TypeDef*)_i2c->DMAx, _i2c->Channel);
	LL_DMA_SetMemoryAddress((DMA_TypeDef*)_i2c->DMAx, _i2c->Channel, (uint32_t)_i2c->buffer);
	LL_DMA_SetDataLength((DMA_TypeDef*)_i2c->DMAx, _i2c->Channel, (uint32_t)_i2c->len);
	LL_DMA_EnableChannel((DMA_TypeDef*)_i2c->DMAx, _i2c->Channel);
	LL_I2C_GenerateStartCondition(_i2c->i2c);
}
...
void I2C_EV_IRQ_DMA_CallBack(I2C_DMA_Conn_t *_i2c) {
	...
	// EV6 Address sent. Clear: read SR1, read SR2
		...
		if (_i2c->mode == I2C_MODE_WRITE) {//work correct
			(void) _i2c->i2c->SR2;	//Read SR2, clear ADDR
			_i2c->i2c->DR = _i2c->reg;//It's FIRST byte			//Send first byte reg address
...
void I2C_ERR_DMA_CallBack(I2C_IRQ_Conn_t *_i2c) {
	volatile uint32_t I2C_SR1 = 0;
	volatile uint32_t I2C_SR2 = 0;
	_i2c->status = PORT_ERROR;
	I2C_SR1 = _i2c->i2c->SR1;/* Read the I2C1 status register */
