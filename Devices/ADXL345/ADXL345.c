...
static const float my_gravity = 9.80665; // m/s^2
...
uint8_t ADXL345_Init(I2C_Connection_t *_i2c, ADXL345_t *dev) {
	if (_i2c->status == PORT_FREE) {
		...
		case 0:{//setup TAP FreeFall Threshold and Offsets 0x1D..0x2A registers
			...
			FIFO_PutMulti(_i2c->buffer, cfg, 14);
			_i2c->len = 14;
		...
		case 1:{ //setup power samplerate interrupt etc
			...
			FIFO_PutMulti(_i2c->buffer, cfg, 4);
			_i2c->len = 4;
			_i2c->mode = I2C_MODE_WRITE;
			_i2c->step = 2;
			break;}
...
uint8_t ADXL345_GetData(I2C_Connection_t *_i2c, ADXL345_t *dev) {
	if (_i2c->status == PORT_FREE) {
		...
		case 1:{ //convert data
			...
			dev->data.X = ADXL345_ConvertData(dev->raw.X, ADXL345_DATA_FORMAT_RANGE_2G);
			dev->raw.Y = (int16_t)CONCAT_BYTES(val[3], val[2]);
			dev->data.Y = ADXL345_ConvertData(dev->raw.Y, ADXL345_DATA_FORMAT_RANGE_2G);
			dev->raw.Z = (int16_t)CONCAT_BYTES(val[5], val[4]);
			dev->data.Z = ADXL345_ConvertData(dev->raw.Z, ADXL345_DATA_FORMAT_RANGE_2G);
			dev->status = OK;
			_i2c->step = 0;
			return 1;
			break;}
