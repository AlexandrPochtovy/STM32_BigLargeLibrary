	...
	switch (dev->step) {
		case 0: //check connection read chip's ID
			if (dev->status == DEVICE_PROCESSING) {
				uint8_t ID = 0;
				st = I2C_ReadOneByte(_i2c, dev->addr, ADXL345_DEVID_REG, &ID);
				if (st == PORT_DONE) {
		...
		case 1: // setup TAP FreeFall Threshold and Offsets 0x1D..0x2A registers
			if (dev->status == DEVICE_PROCESSING) {
				...
				st = I2C_WriteBytes(_i2c, dev->addr, ADXL345_THRESH_TAP_REG, data, 14);
				if (st == PORT_DONE) {
					_i2c->status = PORT_BUSY;
					dev->step = 2;
					}
			...
			break;
		case 2:
			if (dev->status == DEVICE_PROCESSING) { // setup power samplerate interrupt etc
				uint8_t data[4];
				...
				st = I2C_WriteBytes(_i2c, dev->addr, ADXL345_BW_RATE_REG, data, 4);
				if (st == PORT_DONE) {
					_i2c->status = PORT_BUSY;
					dev->step = 3;
					}
		...
		case 3: // setup data format
			if (dev->status == DEVICE_PROCESSING) {
				st = I2C_WriteOneByte(_i2c, dev->addr, ADXL345_DATA_FORMAT_REG, 0x00);
				if (st == PORT_DONE) {
					_i2c->status = PORT_BUSY;
					dev->step = 4;
					}
		...
		case 4: // setup FIFO
			if (dev->status == DEVICE_PROCESSING) {
				st = I2C_WriteOneByte(_i2c, dev->addr, ADXL345_FIFO_CTL_REG, 0x00);
				if (st == PORT_DONE) {
...
uint8_t ADXL345_GetData(I2C_IRQ_Conn_t *_i2c, ADXL345_t *dev) {
	...
	if (dev->status == DEVICE_PROCESSING) {
		...
		st = I2C_ReadBytes(_i2c, dev->addr, ADXL345_DATAX0_REG, val, ADXL345_DATA_LENGHT);
		if (st == PORT_DONE) {
