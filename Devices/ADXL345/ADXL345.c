		...
		case DEVICE_PROCESSING:
			switch (dev->step) {
				case 0: { // setup TAP FreeFall Threshold and Offsets 0x1D..0x2A registers
					...
					st = I2C_WriteBytes(port, dev->addr, ADXL345_THRESH_TAP_REG, data, 14);
					if (st == PORT_DONE) {
						port->status = PORT_BUSY;
						dev->step = 1;
						}
				...
				case 1:
					...
					st = I2C_WriteBytes(port, dev->addr, ADXL345_BW_RATE_REG, data, 4);
					if (st == PORT_DONE) {
						port->status = PORT_BUSY;
						dev->step = 2;
						}
				...
				case 2: // setup data format
					st = I2C_WriteOneByte(port, dev->addr, ADXL345_DATA_FORMAT_REG, 0x00);
					if (st == PORT_DONE) {
						port->status = PORT_BUSY;
						dev->step = 3;
						}
					break;
				case 3: // setup FIFO
					st = I2C_WriteOneByte(port, dev->addr, ADXL345_FIFO_CTL_REG, 0x00);
					if (st == PORT_DONE) {
...
uint8_t ADXL345_GetData(I2C_IRQ_Conn_t *port, ADXL345_t *dev) {
	...
	switch (dev->status) {
		...
		case DEVICE_PROCESSING: {
			...
			st = I2C_ReadBytes(port, dev->addr, ADXL345_DATAX0_REG, val, ADXL345_DATA_LENGHT);
			if (st == PORT_DONE) {
