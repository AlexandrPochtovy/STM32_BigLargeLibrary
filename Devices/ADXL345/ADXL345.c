...
uint8_t ADXL345_Init(I2C_Connection *_i2c, ADXL345_t *dev) {
	uint8_t cfg[14];
	if (_i2c->status == PORT_FREE) {//&& (_i2c->buffer.lockState == FREE)) { //send setup
		...
		switch (dev->step) {
			...
			case 1: { //setup power samplerate interrupt etc
				...
				cfg[1] = ADXL345_POWER_CTL_MEASURE
...
uint8_t ADXL345_GetData(I2C_Connection *_i2c, ADXL345_t *dev) {
	...
	if (_i2c->status == PORT_FREE) {// && (_i2c->buffer.lockState == FREE)) { //
		_i2c->buffer.lockState = BLOCKED;
		_i2c->addr = dev->addr;
		switch (dev->step) {
			case 0: //get data
			...
			case 1: { //convert data
				...
				dev->raw.X = (int16_t)(((uint16_t)val[1])<< 8 | (uint16_t)val[0]);//CONCAT_BYTES(val[1], val[0]);
				dev->data.X = ADXL345_ConvertData(dev->raw.X);
				dev->raw.Y = (int16_t)(((uint16_t)val[3])<< 8 | (uint16_t)val[2]);//CONCAT_BYTES(val[3], val[2]);
				dev->data.Y = ADXL345_ConvertData(dev->raw.Y);
				dev->raw.Z = (int16_t)(((uint16_t)val[5])<< 8 | (uint16_t)val[4]);//CONCAT_BYTES(val[5], val[4]);
